...
const SejarahIslam: React.FC = () => {
    const { topicId } = useParams<{ topicId: string }>();
    const selectedTopic = topicId ? sejarahData.find(t => t.id === topicId) : null;

    if (selectedTopic) {
        return (
            <section id="sejarah-reader" className="animate-fade-in">
                <Card>
                    <ContentReader 
    ...
    return (
        <section id="sejarah-list" className="animate-fade-in">
            <h2 className="text-3xl font-bold text-gray-800 dark:text-white ApplicationTitle">Khazanah Sejarah Islam</h2>
            <Card>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {sejarahData.map((topic) => (
                        <Link
                            key={topic.id}
                            to={`/sejarah/${topic.id}`}
