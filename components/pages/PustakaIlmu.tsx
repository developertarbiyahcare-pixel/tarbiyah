...
import type { CustomArticle } from '../../types';
...
const PustakaIlmu: React.FC = () => {
    ...
    const [selectedArticle, setSelectedArticle] = useState<CustomArticle | null>(null);

    useEffect(() => {
        try {
            const articlesRaw = localStorage.getItem('tarbiyahCustomArticles');
            if (articlesRaw) {
                setCustomArticles(JSON.parse(articlesRaw));
            }
        } catch (e) {
    ...
    // Simple render for dynamic article reading within the component
    if (selectedArticle) {
        return (
            <section className="animate-fade-in">
                <button onClick={() => setSelectedArticle(null)} className="mb-4 text-primary hover:underline font-semibold flex items-center gap-2">
                    <i className="fas fa-arrow-left"></i> Kembali ke Pustaka
                </button>
                <Card>
                    <h2 className="text-3xl font-bold text-white mb-2">{selectedArticle.title}</h2>
                    <div className="flex gap-4 text-sm text-gray-400 mb-6">
                        <span><i className="fas fa-tag mr-1"></i>{selectedArticle.category}</span>
                        <span><i className="fas fa-calendar mr-1"></i>{new Date(selectedArticle.date).toLocaleDateString('id-ID')}</span>
                    </div>
    ...
    return (
        <section id="pustaka-ilmu" className="animate-fade-in">
            ...
            {customArticles.length > 0 && !searchTerm && (
                <div className="mb-10">
                    <h3 className="text-2xl font-bold text-primary mb-4 border-b-2 border-primary/50 pb-2">Artikel Tambahan (Terbaru)</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {customArticles.map(article => (
                            <div key={article.id} onClick={() => setSelectedArticle(article)} className="bg-slate-700 hover:bg-slate-600 p-4 rounded-lg cursor-pointer transition-colors">
                                <h4 className="font-bold text-lg text-white">{article.title}</h4>
                                <p className="text-xs text-gray-400 mt-1">{article.category} • {new Date(article.date).toLocaleDateString('id-ID')}</p>
                                <p className="text-sm text-gray-300 mt-2 line-clamp-2">{article.content}</p>
                            </div>
                        ))}
                    </div>
                </div>
