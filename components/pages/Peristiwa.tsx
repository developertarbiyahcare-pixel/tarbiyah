...
import { peristiwaData } from '../../data/peristiwa';

const Peristiwa: React.FC = () => {
    const { topicId } = useParams<{ topicId: string }>();
    const selectedTopic = topicId ? peristiwaData.find(t => t.id === topicId) : null;

    if (selectedTopic) {
        return (
            <section id="peristiwa-reader" className="animate-fade-in">
                <Card>
                    <ContentReader 
    ...
    return (
        <section id="peristiwa-list" className="animate-fade-in">
            <h2 className="text-3xl font-bold mb-6 text-gray-800 dark:text-white">Peristiwa Lainnya dalam Islam</h2>
            <Card>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {peristiwaData.map((topic) => (
                        <Link
                            key={topic.id}
                            to={`/peristiwa/${topic.id}`}
